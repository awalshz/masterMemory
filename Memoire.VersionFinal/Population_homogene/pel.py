...
class Pel:
    def __init__(self, age, encours0=1):
        '''
        ...
        self.encours = self.get_encours()

    def taux_clotures(self):
        tx_cl = []
        for simu in simu_euribor3m:
            tx_cl_i = [tx_clotures(self.age + i, simu[i] - self.taux_pel)
    ...
    def provision(self):
        return (DF * (self.encours - self.encours.min(axis=0)) *
...
if __name__ == '__main__':
    ...
    pel = Pel(20)
    t = np.arange(180)
    plt.plot(t, pel.encours.max(axis=0), color="blue",
             linewidth=1.5, linestyle="-", label="encours probable")
    plt.plot(t, pel.encours.min(axis=0), color="red",
