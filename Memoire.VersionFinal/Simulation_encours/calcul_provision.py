...
if __name__ == '__main__':
    ...
    for age in range(179):
        ...
        strate = Pel(age=age, encours0=stock_pel['encours'].iloc[age],
                     erreurs=True)
        loi_provision += [strate.provision(q=2.5)]
        time1 = time.time()
        print(f'Done in {time1 - time0:2f} seconds \n')

    np.savetxt('loi_provision_e', np.array(loi_provision))

else:
    loi_provision = np.loadtxt('loi_provision_e')
...
pourcentage = provision_total.sum() / montant_total
